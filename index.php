<html>
    ...
    <body>
        <ul>
            <li><a href="escape-minify">Escape Minify</a><?php echo 'test'; ?></li>
        </ul>
        <script src="http://code.jquery.com/jquery-2.1.4.min.js"></script>
        <script>
            window.onhashchange = function() {
                if (window.location.href.indexOf('#') >= 0 && window.location.href.replace(/.*#/, '') != '') {hidePage(true, window.location.href.replace(/.*#/, '').replace(/\//, ''))}else {hidePage()};
            };
            $(document).ready(function() {
                if(window.location.href.indexOf('#') >= 0 && window.location.href.replace(/.*#/, '') != '') { 
                    showPage(window.location.href.replace(/.*#/, '').replace(/\//, ''));
                };
            ...
            function showPage(url) {
                ...
                window.history.pushState(url, url.replace(/-/, ' ').replace(/_/, ' ').toLowerCase().replace(/(\b)([a-zA-Z])/g, function(a){return a.toUpperCase()}), url+'/');
                ...
                setTimeout(function() {
                    var entireStyle = 'display:block;pointer-events:all;position:fixed;z-index:1;'+s[0].replace(/_.*/, '')+':' + s[0].replace(/.*_/, '')+';'+s[1].replace(/_.*/, '')+':'+s[1].replace(/.*_/, '')+';margin:'+s[2]+';'+borders()+'border-radius:'+s[7]+';padding:'+s[8]+';width:'+s[9]+';height:'+s[10]+';font-family:'+s[11]+';font-size:'+s[12]+';color:'+s[13]+';text-shadow:'+s[14]+';cursor:'+s[15]+';text-align:'+s[16]+';'+backgroundsReturn;
                    //$('iframe').contents().find('.backbutton').attr('style', entireStyle);
                    $('iframe').contents().find('body').append('<div class="backbutton" style="'+entireStyle+'">Back</div>');
                    $('iframe').contents().find('.backbutton').click(function() {hidePage()});
                    if (hAllow) {
                        var entireStyleHover = 'display:block;pointer-events:all;position:fixed;z-index:1;'+h[0].replace(/_.*/, '')+':' + h[0].replace(/.*_/, '')+';'+h[1].replace(/_.*/, '')+':'+h[1].replace(/.*_/, '')+';margin:'+h[2]+';'+bordersHover()+'border-radius:'+h[7]+';padding:'+h[8]+';width:'+h[9]+';height:'+h[10]+';font-family:'+h[11]+';font-size:'+h[12]+';color:'+h[13]+';text-shadow:'+h[14]+';cursor:'+h[15]+';text-align:'+h[16]+';'+backgroundsReturnHover;
                        /*$('iframe').contents().find('.backbutton').hover(function() {
                            $('iframe').contents().find('.backbutton').attr('style', entireStyleHover);
                        }, function() {
                            $('iframe').contents().find('.backbutton').attr('style', entireStyle);
                        });*/
                    }
                ...
                $('iframe').on('load', function() {
                    //$('iframe').contents().find('body').append('<div class="backbutton">Back</div>');
            ...
            function hidePage(show, url) {
                $('title').text('Projects');
                window.history.pushState('mainPage', 'Projects', '/');
                if ($('iframe').length) {
                    $('iframe').contents().find('.github').css('opacity', '0');
                    $('iframe').animate({
                        width: '0px',
                        ...
                        $('iframe').contents().find('.backbutton').remove();
                        $('iframe').remove();
                    });
                    if (show) setTimeout(function() {showPage(url)}, 1000);
                }else if (show) showPage(url);
            };
